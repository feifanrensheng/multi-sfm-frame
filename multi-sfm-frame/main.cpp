...
void match_current_frame(Mat& frame, Mat& train_descriptors, vector<Point3f>& structure, Mat& K, Mat& R, Mat& T)
{
	...
	for (size_t r = 0; r < knn_matches.size(); ++r)
	{
		if (
			...
			knn_matches[r][0].distance > 10.0f
			)
			continue;
...
int main( int argc, char** argv)
{
    ...
    vector<string> image_names = {"../001.jpg", "../002.jpg", "../003.jpg", "../004.jpg", "../005.jpg", "../006.jpg", "../007.jpg", "../008.jpg"};
    //
    CameraCalibration calibration(1399.31f, 1396.13f, 656.23, 532.98);
    Mat K(Matx33d(
        1399.31, 0, 656.23,
        0, 1396.13, 532.98,
        0, 0, 1));
	...
	for(;;) {
		...
		current_frame(frame, train_descriptors, structure, K, R, T);
		processVideo(calibration, cap);
		
	}
	*/
	processVideo(calibration, cap, train_descriptors, structure );
